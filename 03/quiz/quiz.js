...
function initialization() {
    ...
    for(var i=0; i<todayPhoto.length; i++){
        str[str.length - 1] += imageContent(i);
        if(i % 3 == 2) {
            str.push('');
        }
    }
...
function showPage() {
    container.innerHTML = str[index];   
    rownum.innerText = `${index + 1} / ${str.length}`;
}
